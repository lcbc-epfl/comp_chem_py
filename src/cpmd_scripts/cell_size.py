...
Two options are available:
    ...
    or reading from gromos format.

usage::

    python cell_size.py myfile.xyz

In the case of gromos format, a list of the names of the residues
in the QM region has to be given (when asked by the script).
...
if __name__=="__main__":
   ...
   print("Raw max diff (AA)  : {:12.6f} {:12.6f} {:12.6f} ".format(*diff*BOHR_TO_ANG) )
   print("Raw max diff (a.u.): {:12.6f} {:12.6f} {:12.6f} ".format(*diff) )
   print("")
   diff = (diff+7)
   print("Cell size    (AA)  : {:12.6f} {:12.6f} {:12.6f} ".format(*diff*BOHR_TO_ANG) )
   print("Cell size    (a.u.): {:12.6f} {:12.6f} {:12.6f} ".format(*diff))
   print("")
   dim = np.amax(diff)
   print("Recommended (cubic) cell dimmension = {:9.3f} (AA)".format(dim*BOHR_TO_ANG) )
